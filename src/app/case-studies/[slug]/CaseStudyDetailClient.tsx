...
const caseStudyResources: Record<string, { project: string; article: string }> = {
  "medical-document-upload-automation-case-study": {
    project: "/projects/medical-document-upload-automation",
    article: "/blog/medical-document-upload-automation",
  },
  "telemedicine-encounter-automation-case-study": {
    project: "/projects/telemedicine-appointment-encounter-automation",
    article: "/blog/telemedicine-workflow-automation",
  },
  "insurance-eligibility-verification-automation-case-study": {
    project: "/projects/insurance-eligibility-verification-automation",
    article: "/blog/insurance-verification-clinic-delay",
  },
  "insurance-pharmacy-data-extraction-automation-case-study": {
    project: "/projects/insurance-pharmacy-data-extraction-automation",
    article: "/blog/healthcare-data-extraction",
  },
  "ai-soap-notes-entry-automation-case-study": {
    project: "/projects/ai-soap-notes-automation-emr-integration",
    article: "/blog/ai-soap-notes-automation",
  },
  "emr-patient-automation-case-study": {
    project: "/projects/automated-patient-registration-appointment-emr-automation",
    article: "/blog/emr-automation-clinic-operations",
  },
};
...
export default function CaseStudyDetailClient({ initialStudy }: { initialStudy: CaseStudy }) {
  ...
  const [study, setStudy] = useState<CaseStudy | null>(initialStudy);
  const [loading, setLoading] = useState(false);
  const [currentSlide, setCurrentSlide] = useState(0);

  useEffect(() => {
    if (initialStudy && initialStudy.slug === slug) {
      return;
    }
    ...
    >
      ...
      <section style={{ padding: "32px 20px 72px", textAlign: 'center' }}>
        <motion.div whileInView={{ opacity: 1, scale: 1 }} initial={{ opacity: 0, scale: 0.95 }} style={{ background: isDark ? 'rgba(255,255,255,0.03)' : '#f1f5f9', padding: '80px 40px', borderRadius: '60px', border: `1px solid ${colors.border}` }}>
            ...
            <nav aria-label="Related case study resources" style={{ marginBottom: "28px", lineHeight: 1.8 }}>
              <Link href="/services-details">Healthcare automation services</Link>
              {caseStudyResources[study.slug] && (
                <>
                  {" · "}<Link href={caseStudyResources[study.slug].project}>View the implemented system</Link>
                  {" · "}<Link href={caseStudyResources[study.slug].article}>Read the related guide</Link>
                </>
              )}
              {" · "}<Link href="/contact">Contact EaseWorkflow</Link>
            </nav>
            <motion.button 
